        ...
        <div class="card flex items-center p-5">
            <nav class=" w-full  flex items-center justify-between" aria-label="Global">
                    ...
                    <li class="relative">
                        <nav class="w-full rounded-md" aria-label="breadcrumb">
                            <ol class="list-reset ms-2 flex">
                                ...
                                <li>
                                    <a href="#"
                                        class="motion-reduce:transition-none-none
                                         text-black/60 transition duration-200 hover:text-black/80 hover:ease-in-out
                                          focus:text-black/80 active:text-black/80 motion-reduce:transition
                                          motion-reduce:transition-none ">Affiliation Types
                                        </a>
                                </li>
                                <li>
                                    <span class="mx-2 text-black/60">/</span>
                ...
                <div class="flex items-center gap-4">
                    <a href="{{ route('admin.affiliationType.index') }}"
                        class="btn font-medium bg-blue-600 hover:bg-red-600 py-1 text-white" aria-current="page">Affiliation Types List</a>
                </div>
            </nav>
        </div>
        <div class="">
            <div class="flex flex-col gap-6">
                ...
                <div class="card">
                    <div class="card-body">
                        <form class="flex flex-col gap-6" action="{{route('admin.affiliationType.update',$affiliationType)}}" method="POST"
                              enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="grid grid-cols-2 gap-4">
                                <x-forms.TextInput
                                    label="Affilitation Type Name "
                                    id="title"
                                    name="title"
                                    value="{{ old('title',$affiliationType->title) }}"
